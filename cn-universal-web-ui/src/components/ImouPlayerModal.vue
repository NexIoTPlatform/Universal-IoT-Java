  ...
  <div style="width: 100vw; height: 100vh; background: #000; display: flex; flex-direction: column;">
    ...
    <div
      ...
      <div style="display: flex; align-items: center; gap: 8px; margin-left: auto;">
        <a-input size="small" v-model="form.deviceId" placeholder="设备ID"
                 style="width: 160px; background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2);"/>
        <a-input-number size="small" v-model="form.channelId" placeholder="通道" :min="0"
                        style="width: 100px; background: rgba(255,255,255,0.1); border: 1px solid rgba(255,255,255,0.2);"/>
        <a-input size="small" v-model="form.token" placeholder="Token"
    ...
    <div
      ...
      <!-- 音频控制 -->
      <div style="display: flex; align-items: center; gap: 6px; margin-right: 16px;">
        <a-button size="small" @click="() => handleVolume(1)"
                  style="background: #722ed1; border-color: #722ed1; color: #fff;">
          <a-icon type="sound"/>
        </a-button>
        <a-button size="small" @click="() => handleVolume(0)"
      ...
      <!-- 对讲控制 -->
      <div style="display: flex; align-items: center; gap: 6px; margin-right: 16px;">
        <a-button size="small" @click="handleStartTalk"
                  style="background: #fa8c16; border-color: #fa8c16; color: #fff;">
          <a-icon type="phone"/>
        </a-button>
        <a-button size="small" @click="handleStartTalkVideo"
      ...
      <!-- 录制控制 -->
      <div style="display: flex; align-items: center; gap: 6px; margin-right: 16px;">
        <a-button size="small" @click="handleStartRecord"
                  style="background: #eb2f96; border-color: #eb2f96; color: #fff;">
          <a-icon type="video-camera-add"/>
        </a-button>
        <a-button size="small" @click="handleStopRecord"
                  style="background: #722ed1; border-color: #722ed1; color: #fff;">
          <a-icon type="stop"/>
        </a-button>
      ...
      <div style="display: flex; align-items: center; gap: 6px; margin-right: 16px;">
        ...
        <a-button size="small" @click="handleExitFullScreen"
    ...
    <!-- 状态信息栏 -->
    <div
      style="padding: 8px 16px; background: linear-gradient(135deg, #0a0a0a 0%, #1a1a1a 100%); color: #ccc; font-size: 12px; border-top: 1px solid #333;">
      <div style="display: flex; justify-content: space-between; align-items: center;">
        <div style="display: flex; gap: 20px;">
          <div>
            <span style="color: #1890ff;">状态:</span>
            <span
              :style="{ color: statusText === 'playing' ? '#52c41a' : statusText === 'error' ? '#ff4d4f' : '#faad14' }">{{
                statusText || '未初始化'
              }}</span>
          </div>
  ...
  methods: {
    ...
    toggleMuted() {
      try {
        ...
        setTimeout(() => this.player?.volume?.(1), 300)
      } catch (e) {
      }
    },
    handleFullScreen() {
