      ...
      <a-card :bordered="false">
        ...
        <a-table :loading="loading" :size="tableSize" rowKey="id" :columns="columns"
          ...
          <div slot="status" slot-scope="text, record" class="status-cell">
            <div :class="{ 'status-badge draft': record.status === 'draft', 'status-badge deployed': record.status === 'deployed', 'status-badge stopped': record.status === 'stopped' }">
          ...
          <span slot="operation" slot-scope="text, record" class="operation-buttons">
            ...
            <a @click="handleDeploy(record)" v-if="record.status === 'draft'" v-hasPermi="['rulego:chain:deploy']" class="operation-btn">
              <a-icon type="upload"/>
              部署
            </a>
            <a-divider type="vertical" v-if="record.status === 'deployed'" v-hasPermi="['rulego:chain:stop']"/>
            <a @click="handleStop(record)" v-if="record.status === 'deployed'" v-hasPermi="['rulego:chain:stop']" class="operation-btn">
              <a-icon type="pause"/>
              停止
            </a>
            ...
            <a style="color:#F53F3F" @click="handleDelete(record)" v-hasPermi="['rulego:chain:remove']" class="operation-btn">
              <a-icon type="delete"/>
              删除
            </a>
          </span>
...
<script>
import { listChain, getChain, delChain, addChain, updateChain, deployChain, stopChain, syncChain, getDesignerUrl } from "@/api/rulego/chain";
