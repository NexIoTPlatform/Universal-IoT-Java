  ...
  <page-header-wrapper>
            ...
            <a-col :md="6" :sm="24">
              <a-form-item label="内容类型">
                <a-select v-model="contentType" placeholder="请选择内容类型" style="width: 100%">
                  ...
                  <a-select-option value="units">设备单元</a-select-option>
      ...
      <div class="content-wrapper">
        ...
        <div class="right-content">
          ...
          <div v-if="contentType === 'devices'">
            ...
            >
              ...
              <span slot="action" slot-scope="text, record">
                ...
                <a @click="viewUnits(record)">单元</a>
              </span>
            </a-table>
          </div>

          <!-- 设备单元列表 -->
          <div v-if="contentType === 'units'">
            <a-descriptions size="small" :column="4" style="margin-bottom: 16px;" v-if="currentDevice">
              ...
              <a-descriptions-item label="单元数">{{ units.length }}</a-descriptions-item>
            </a-descriptions>
          ...
          <div v-if="contentType === 'channels'">
            <a-descriptions size="small" :column="4" style="margin-bottom: 16px;" v-if="currentUnit">
              <a-descriptions-item label="单元名称">{{ currentUnit.unitName }}</a-descriptions-item>
              <a-descriptions-item label="单元ID">{{ currentUnit.unitId }}</a-descriptions-item>
              <a-descriptions-item label="状态">
                <a-badge :status="currentUnit.status === 'ON' ? 'success' : 'default'" 
                         :text="currentUnit.status === 'ON' ? '在线' : '离线'" />
              </a-descriptions-item>
              <a-descriptions-item label="通道数">{{ channels.length }}</a-descriptions-item>
            </a-descriptions>
              ...
              <a-col :span="6">
                ...
                <a-card title="通道信息" size="small" :bordered="false" style="margin-top: 16px;" v-if="currentChannel">
                  <a-descriptions size="small" :column="1">
                    <a-descriptions-item label="通道名称">{{ currentChannel.channelName }}</a-descriptions-item>
                    <a-descriptions-item label="通道编码">{{ currentChannel.channelCode }}</a-descriptions-item>
                    <a-descriptions-item label="状态">
    ...
    <a-modal v-model="deviceModalVisible" title="设备详情" width="800px" :footer="null">
      <a-descriptions size="small" :column="2" v-if="selectedDevice">
        ...
        <a-descriptions-item label="设备序列号">{{ selectedDevice.deviceSn || '-' }}</a-descriptions-item>
        <a-descriptions-item label="状态">
...
export default {
  ...
  data() {
    return {
      ...
      units: [],
      unitColumns: [
        { title: '单元ID', dataIndex: 'unitId', width: 180 },
        { title: '单元名称', dataIndex: 'unitName', width: 150, ellipsis: true },
        { title: '单元类型', dataIndex: 'unitType', width: 100 },
        { title: '状态', dataIndex: 'status', width: 80, scopedSlots: { customRender: 'status' } },
        { title: '通道数', dataIndex: 'channelCount', width: 80 },
      ...
      channels: [],
      channelColumns: [
        { title: '通道编码', dataIndex: 'channelCode', width: 180 },
        { title: '通道名称', dataIndex: 'channelName', width: 150, ellipsis: true },
        { title: '通道类型', dataIndex: 'channelType', width: 100 },
        { title: '状态', dataIndex: 'status', width: 80, scopedSlots: { customRender: 'status' } },
  ...
  methods: {
    ...
    viewUnits(device) {
      ...
      // 从配置中解析units
      this.units = this.parseUnits(device)
      this.contentType = 'units'
    },
    parseUnits(device) {
      // 解析大华ICC特有的units结构
      try {
        if (device.configuration) {
          const config = JSON.parse(device.configuration)
          return config.units || []
        }
      } catch (e) {
        console.error('解析units失败:', e)
      }
      return []
    },
    viewChannels(unit) {
      this.currentUnit = unit
      this.channels = unit.channels || []
