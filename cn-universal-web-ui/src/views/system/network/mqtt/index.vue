<template>
  <page-header-wrapper>
    <a-card :bordered="false">
          ...
          <a-card hoverable class="network-card">
            ...
            <div class="card-body">
              ...
              <div class="card-row">
                ...
                绑定产品：<span class="product-count-inline"><a-button v-if="item.type === 'MQTT_SERVER' && item.bindMqttServerProductCount > 0" 
      ...
      <!-- 绑定产品弹窗 -->
        <a-modal
          :title="`${currentNetwork.name} - 绑定产品列表`"
          ...
          <div v-if="currentBindProducts && currentBindProducts.length > 0">
            <div v-for="(item, index) in currentBindProducts" :key="index" style="margin-bottom: 16px;">
              <div style="display: flex; gap: 16px;">
                <div style="flex: 1;">
                  <a-descriptions :column="1" size="small">
                    ...
                    <a-descriptions-item label="设备节点" v-if="item.deviceNode">
                      {{ item.deviceNode }}
                    </a-descriptions-item>
                    <a-descriptions-item label="协议类型" v-if="item.thirdPlatform">
                      {{ item.thirdPlatform }}
                    </a-descriptions-item>
                  </a-descriptions>
                </div>
                <div style="width: 150px; height: 150px; flex-shrink: 0; display: flex; align-items: center; justify-content: center;">
                  <div v-if="getProductImage(item)" style="width: 100%; height: 100%; border: 1px solid #d9d9d9; border-radius: 8px; overflow: hidden;">
              ...
              <div v-if="index < currentBindProducts.length - 1" style="margin-top: 16px; border-bottom: 1px solid #f0f0f0;"></div>
            </div>
          </div>
