        ...
        <div class="video-panel">
          ...
          <div class="video-controls">
            ...
            <div class="controls-left">
              ...
              <!-- 通道选择 -->
              <template v-if="hasMultipleChannels">
                <!-- 分隔线 -->
              ...
              <div class="protocol-selector">
                ...
                <a-select v-model="previewOptions.protocol" @change="onProtocolChange" size="small"
...
export default {
  ...
  computed: {
    ...
    hasMultipleChannels() {
      try {
        // 解析configuration字段（可能是字符串或对象）
        let configuration = this.deviceInfo?.configuration
        if (typeof configuration === 'string') {
          configuration = JSON.parse(configuration)
        }

        console.log("通道数据情况:", configuration)

        return configuration?.channelList &&
          Array.isArray(configuration.channelList) &&
          configuration.channelList.length > 1
      } catch (e) {
        console.error('解析configuration失败:', e)
        return false
      }
    },

    // 获取解析后的configuration对象
    parsedConfiguration() {
      try {
        let configuration = this.deviceInfo?.configuration
        if (typeof configuration === 'string') {
  ...
  methods: {
    ...
    // 初始化通道ID
    initChannelId() {
      const configuration = this.parsedConfiguration
      if (configuration?.channelList && configuration.channelList.length > 0) {
        // 默认使用第一个通道的channelId，如果第一个通道的channelId为0，则使用0
        this.previewOptions.channelId = configuration.channelList[0].channelId || 0
      } else {
        // 如果没有通道信息，默认使用0
        this.previewOptions.channelId = 0
      }
    },
    // 获取状态样式类
    getStatusClass() {
    ...
    async onChannelChange() {
      console.log('通道切换:', this.previewOptions.channelId)
      if (this.playerVisible) {
        await this.restartPreviewWithNewToken()
      }
    },
    // 重新开始预览
    ...
    // 通道切换时重新获取token并开始预览
    async restartPreviewWithNewToken() {
      try {
        this.videoStatus = '正在切换通道...'
        this.destroyPlayer()
        this.playerVisible = false

        // 延迟一下再重新开始预览，确保获取新的token
        setTimeout(async () => {
          await this.startPreview()
        }, 500)
      } catch (e) {
        console.error('通道切换失败:', e)
        this.videoStatus = '通道切换失败'
      }
    },
    resetVideoState() {
      this.playerVisible = false
        ...
        const requestData = {
          ...
          function: {
            ...
            data: {
              ...
              channelId: this.previewOptions.channelId
            }
          }
        }
...
.channel-selector .control-label,
...
.channel-selector .channel-select .ant-select-selector {
  background: rgba(255, 255, 255, 0.1) !important;
  border: 1px solid rgba(255, 255, 255, 0.2) !important;
  border-radius: 4px !important;
  height: 26px !important;
...
.channel-selector .channel-select .ant-select-selection-item {
  color: rgba(255, 255, 255, 0.8) !important;
  line-height: 24px !important;
  font-size: 11px !important;
}

.channel-selector .channel-select .ant-select-arrow {
  color: rgba(255, 255, 255, 0.6) !important;
}

.channel-selector .channel-select:hover .ant-select-selector {
  border-color: #1890ff !important;
}

/* 通道选择器下拉选项样式 */
.channel-selector .ant-select-dropdown {
...
.channel-selector .ant-select-item {
  color: rgba(255, 255, 255, 0.8) !important;
  background: transparent !important;
}

.channel-selector .ant-select-item:hover {
  background: rgba(24, 144, 255, 0.2) !important;
  color: #fff !important;
}

.channel-selector .ant-select-item-option-selected {
