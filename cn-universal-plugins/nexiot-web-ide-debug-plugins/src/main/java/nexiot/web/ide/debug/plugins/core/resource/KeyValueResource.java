...
public abstract class KeyValueResource implements Resource {
  ...
  public KeyValueResource(
      String separator, String path, boolean readonly, KeyValueResource parent) {
    this.separator = separator;
    this.path = path;
    this.parent = parent;
  ...
  public final boolean renameTo(Resource resource) {
    ...
    if (!(resource instanceof KeyValueResource)) {
      throw new IllegalArgumentException(
          "无法将" + this.getAbsolutePath() + "重命名为:" + resource.getAbsolutePath());
    }
    ...
    if (resource.isDirectory()) {
      ...
      oldKeys.forEach(
          oldKey -> mappings.put(oldKey, targetResource.path + oldKey.substring(keyLen)));
