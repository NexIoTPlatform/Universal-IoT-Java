package nexiot.web.ide.debug.plugins.datasource.service;
...
public class DataSourceMagicDynamicRegistry extends AbstractMagicDynamicRegistry<DataSourceInfo> {
  ...
  private static final Logger logger =
      LoggerFactory.getLogger(DataSourceMagicDynamicRegistry.class);

  private static final ClassLoader CLASSLOADER =
      DataSourceMagicDynamicRegistry.class.getClassLoader();
  ...
  public DataSourceMagicDynamicRegistry(
      MagicResourceStorage<DataSourceInfo> magicResourceStorage,
  ...
  protected boolean register(MappingNode<DataSourceInfo> mappingNode) {
    ...
    magicDynamicDataSource.put(
        info.getId(), info.getKey(), info.getName(), datasource, info.getMaxRows());
    return true;
  }
  ...
  private DataSource createDataSource(
      Class<? extends DataSource> dataSourceType, Map<String, Object> properties) {
  ...
  public List<DataSourceInfo> defaultMappings() {
    return magicDynamicDataSource.datasourceNodes().stream()
            ...
            it -> {
              ...
              dataSourceInfo.setName(
                  StringUtils.defaultIfBlank(
                      it.getName(), StringUtils.defaultIfBlank(it.getKey(), "默认数据源")));
