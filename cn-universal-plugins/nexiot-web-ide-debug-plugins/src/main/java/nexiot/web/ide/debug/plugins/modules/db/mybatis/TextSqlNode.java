package nexiot.web.ide.debug.plugins.modules.db.mybatis;

import cn.universal.core.engine.functions.StreamExtension;
import cn.universal.core.engine.parsing.GenericTokenParser;
...
public class TextSqlNode extends SqlNode {
  ...
  private static final GenericTokenParser CONCAT_TOKEN_PARSER =
      new GenericTokenParser("${", "}", false);

  private static final GenericTokenParser REPLACE_TOKEN_PARSER =
      new GenericTokenParser("#{", "}", true);

  private static final GenericTokenParser IF_TOKEN_PARSER = new GenericTokenParser("?{", "}", true);

  private static final GenericTokenParser IF_PARAM_TOKEN_PARSER =
      new GenericTokenParser("?{", ",", true);

  private static final GenericTokenParser OUT_PARAM_TOKEN_PARSER =
      new GenericTokenParser("@{", ",", true);

  private static final GenericTokenParser OUT_TOKEN_PARSER =
      new GenericTokenParser("@{", "}", true);

  private static final GenericTokenParser TYPE_TOKEN_PARSER =
      new GenericTokenParser(",", "}", true);
  ...
  public static String parseSql(String sql, Map<String, Object> varMap, List<Object> parameters) {
    ...
    // 处理?{}参数
    sql =
        IF_TOKEN_PARSER.parse(
            sql.trim(),
            text -> {
              AtomicBoolean ifTrue = new AtomicBoolean(false);
              String val =
                  IF_PARAM_TOKEN_PARSER.parse(
                      "?{" + text,
                      param -> {
                        ifTrue.set(
                            BooleanLiteral.isTrue(ScriptManager.executeExpression(param, varMap)));
                        return null;
                      });
    ...
    // 处理${}参数
    sql =
        CONCAT_TOKEN_PARSER.parse(
            sql, text -> String.valueOf(ScriptManager.executeExpression(text, varMap)));
    // 处理#{}参数
    sql =
        REPLACE_TOKEN_PARSER.parse(
            sql,
            text -> {
              ...
              OUT_PARAM_TOKEN_PARSER.parse(
                  ...
                  param -> {
                    ...
                    if (index > 0) {
                      ...
                      INOUT_TOKEN_PARSER.parse(
                          "@{" + param,
                          inoutParam -> {
                            SqlInOutParameter p = new SqlInOutParameter(inoutParam, sqlType.get());
                            parameters.add(
                                new SqlParameterValue(
                                    p, ScriptManager.executeExpression(value, varMap)));
                            return null;
                          });
                    } else {
