package nexiot.web.ide.debug.plugins.core.logging;
...
public class Formatter {
  ...
  private static final CachingDateFormatter CACHING_DATE_FORMATTER =
      new CachingDateFormatter("yyyy-MM-dd HH:mm:ss.SSS");
  ...
  private Formatter alignment(
      String value, int min, int max, boolean leftTruncate, boolean leftPad) {
    if (value == null) {
      if (0 < min) {
        spacePad(buf, min);
      }
  ...
  private static class TargetLengthBasedClassNameAbbreviator {
        ...
        if (i == 0) {
          ...
          buf.append(
              fqClassName.substring(
                  dotIndexesArray[i - 1], dotIndexesArray[i - 1] + lengthArray[i]));
        }
    ...
    void computeLengthArray(
        final String className, int[] dotArray, int[] lengthArray, int dotCount) {
