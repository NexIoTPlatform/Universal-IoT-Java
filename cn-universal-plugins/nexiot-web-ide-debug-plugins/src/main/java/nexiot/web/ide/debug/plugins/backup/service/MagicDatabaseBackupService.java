package nexiot.web.ide.debug.plugins.backup.service;

import nexiot.web.ide.debug.plugins.backup.model.Backup;
...
public class MagicDatabaseBackupService implements MagicBackupService {
  ...
  public MagicDatabaseBackupService(JdbcTemplate template, String tableName) {
    ...
    this.INSERT_SQL =
        String.format(
            "insert into %s(%s,content) values(?,?,?,?,?,?,?)", tableName, DEFAULT_COLUMNS);
    this.FIND_BY_ID =
        String.format(
            "select %s from %s where id = ? order by create_date desc", DEFAULT_COLUMNS, tableName);
    this.DELETE_BY_ID = String.format("delete from %s where id = ?", tableName);
    this.FIND_BY_TAG =
        String.format(
            "select %s from %s where tag = ? order by create_date desc",
            DEFAULT_COLUMNS, tableName);
    this.FIND_BY_TIMESTAMP =
        String.format(
            "select %s from %s where create_date < ? order by create_date desc",
            DEFAULT_COLUMNS, tableName);
    this.DELETE_BY_TIMESTAMP = String.format("delete from %s where create_date < ?", tableName);
    this.FIND_BY_ID_AND_TIMESTAMP =
        String.format("select * from %s where id = ? and create_date = ?", tableName);
  }
  ...
  @EventListener(
      condition =
          "#event!=null &&  #event.source != T(nexiot.web.ide.debug.plugins.core.config.Constants)"
              + ".EVENT_SOURCE_NOTIFY")
  ...
  public void onFolderEvent(GroupEvent event) {
    ...
    doBackup(
        group.getId(), JsonUtils.toJsonBytes(group), group.getName(), group.getType() + "-group");
  }
