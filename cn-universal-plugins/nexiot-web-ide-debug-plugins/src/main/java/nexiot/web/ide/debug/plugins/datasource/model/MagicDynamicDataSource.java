package nexiot.web.ide.debug.plugins.datasource.model;
...
import nexiot.web.ide.debug.plugins.utils.Assert;
...
public class MagicDynamicDataSource {
   ...
   */
  public void put(
      String id, String dataSourceKey, String datasourceName, DataSource dataSource, int maxRows) {
    if (dataSourceKey == null) {
      dataSourceKey = "";
    }
    logger.info("注册数据源：{}", StringUtils.isNotBlank(dataSourceKey) ? dataSourceKey : "default");
    DataSourceNode node =
        this.dataSourceMap.put(
            dataSourceKey,
            new DataSourceNode(dataSource, dataSourceKey, datasourceName, id, maxRows));
    if (node != null) {
      node.close();
    }
    if (id != null) {
      ...
      this.dataSourceMap.entrySet().stream()
          .filter(
              it ->
                  id.equals(it.getValue().getId())
                      && !finalDataSourceKey.equals(it.getValue().getKey()))
          .findFirst()
          .ifPresent(
              it -> {
  ...
  public DataSourceNode getDataSource(String datasourceKey) {
    ...
    DataSourceNode dataSourceNode = dataSourceMap.get(datasourceKey);
