...
import cn.universal.core.engine.asm.ClassReader;
...
public class ClassScanner {
  ...
  public static List<String> latestJdkScan() throws IOException {
    ...
    return Arrays.asList(resources).parallelStream()
        .map(
            it -> {
              try {
  ...
  private static Set<String> addJava8Library() {
    try {
      // 直接反射调用..
      Object classpath =
          Class.forName("sun.misc.Launcher").getMethod("getBootstrapClassPath").invoke(null);
  ...
  private static List<String> scanDirectory(File dir, String packageName) {
    ...
    if (files != null) {
      for (File file : files) {
        ...
        if (file.isDirectory()) {
          classes.addAll(
              scanDirectory(file, packageName == null ? name : packageName + "." + name));
      ...
      while ((entry = zis.getNextEntry()) != null) {
        if (!entry.getName().contains("META-INF")) {
          ...
          if (isClass(className)) {
            classes.add(
                filterFullName(className.substring(0, className.length() - 6).replace("/", ".")));
          }
        }
      }
    } catch (IOException ignored) {
