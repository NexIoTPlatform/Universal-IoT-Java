...
public class JarResource implements Resource {
  ...
  public JarResource(
      JarFile jarFile, String entryName, List<JarEntry> entries, boolean inSpringBoot) {
  ...
  public Resource getResource(String name) {
    ...
    return new JarResource(
        this.jarFile,
        entryName,
        entries.stream().filter(it -> it.getName().startsWith(prefix)).collect(Collectors.toList()),
        this,
        this.inSpringBoot);
  }
  ...
  public List<Resource> files(String suffix) {
    return this.entries.stream()
        ...
        .map(
            entry ->
                new JarResource(
                    jarFile, entry.getName(), Collections.emptyList(), this, this.inSpringBoot))
                ...
                new JarResource(
                    ...
                    entries.stream()
                        .filter(
                            item ->
                                item.getName()
                                    .startsWith(PathUtils.replaceSlash(entry.getName() + "/")))
                        .collect(Collectors.toList()),
                    this,
                    this.inSpringBoot))
        .collect(Collectors.toList());
