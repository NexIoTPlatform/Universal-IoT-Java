...
public class DatabaseResource extends KeyValueResource {
  ...
  public void readAll() {
    ...
    String sql =
        String.format(
            "select file_path, file_content from %s where file_path like '%s%%'",
            tableName, this.path);
  ...
  public Set<String> keys() {
    ...
    String sql =
        String.format("select file_path from %s where file_path like '%s%%'", tableName, prefix);
  ...
  public boolean renameTo(Map<String, String> renameKeys) {
    ...
    if (affectedRows > 0 || affectedRows == -2) {
      renameKeys.forEach(
          (oldKey, newKey) -> this.cachedContent.put(newKey, this.cachedContent.remove(oldKey)));
      return true;
    }
    return false;
  }
  ...
  public boolean delete() {
    String sql =
        String.format(
            "delete from %s where file_path = ? or file_path like '%s%%'", tableName, path);
