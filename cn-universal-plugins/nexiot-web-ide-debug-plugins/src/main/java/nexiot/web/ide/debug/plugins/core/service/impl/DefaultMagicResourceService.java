...
import nexiot.web.ide.debug.plugins.core.event.MagicEvent;
...
import nexiot.web.ide.debug.plugins.core.model.PathMagicEntity;
import nexiot.web.ide.debug.plugins.core.model.SelectedResource;
import nexiot.web.ide.debug.plugins.core.model.TreeNode;
import nexiot.web.ide.debug.plugins.core.resource.Resource;
import nexiot.web.ide.debug.plugins.core.resource.ZipResource;
...
import nexiot.web.ide.debug.plugins.utils.IoUtils;
...
public class DefaultMagicResourceService
  ...
  public DefaultMagicResourceService(
      Resource resource,
      List<MagicResourceStorage<? extends MagicEntity>> storages,
    ...
    if (entity != null) {
      ...
      if (group != null) {
        ...
        Map<String, String> pathCacheMap =
            storage.requirePath() ? pathCache.get(storage.folder()) : null;
        ...
        publisher.publishEvent(
            new FileEvent(group.getType(), action, entity, Constants.EVENT_SOURCE_NOTIFY));
      }
    }
    return false;
  }
          ...
          fileCache
              ...
              .forEach(
                  entity -> {
                    Group group = groupCache.get(entity.getGroupId());
                    publisher.publishEvent(
                        new FileEvent(group.getType(), EventAction.LOAD, entity));
                  });
          return null;
        });
  }
  ...
  private void refreshGroup(Resource folder, MagicResourceStorage<? extends MagicEntity> storage) {
    if (storage.allowRoot()) {
      folder
          .files(storage.suffix())
          .forEach(file -> putFile(storage, storage.readResource(file), file));
    } else {
      folder
          .dirs()
          .forEach(
    ...
    return writeLock(
        () -> {
          ...
          GroupEvent event =
              new GroupEvent(
                  group.getType(),
                  group.getId() == null ? EventAction.CREATE : EventAction.SAVE,
                  group);
          if (group.getId() == null || !groupCache.containsKey(group.getId())) {
            ...
            if (storage.requirePath() && !Objects.equals(oldGroup.getPath(), group.getPath())) {
              ...
              List<MagicEntity> entities =
                  treeNode.findTreeNode(it -> it.getId().equals(group.getId())).flat().stream()
                      .flatMap(
                          it ->
                              fileCache.values().stream()
                                  .filter(f -> f.getGroupId().equals(it.getId())))
                      .collect(Collectors.toList());
              for (MagicEntity entity : entities) {
                String newMappingKey = storage.buildKey(entity);
                if (pathCache.get(group.getType()).entrySet().stream()
                    .anyMatch(
                        entry ->
          ...
          // 写入分组信息
          if (groupResource
              .getResource(Constants.GROUP_METABASE)
              .write(JsonUtils.toJsonString(group))) {
            putGroup(group, groupResource);
            TreeNode<Group> treeNode =
                tree(group.getType()).findTreeNode(it -> it.getId().equals(group.getId()));
            // 刷新分组缓存
  ...
  private boolean moveGroup(Group src, String target) {
    ...
    Resource targetResource =
        Constants.ROOT_ID.equals(target)
            ? this.root.getDirectory(storage.folder())
            : groupMappings.get(target);
    // 校验分组名称是否有冲突
    ...
    // 递归找出要移动的文件
    List<MagicEntity> entities =
        treeNode.findTreeNode(it -> it.getId().equals(src.getId())).flat().stream()
            .flatMap(
                it -> fileCache.values().stream().filter(f -> f.getGroupId().equals(it.getId())))
            .collect(Collectors.toList());
    if (storage.requirePath()) {
      for (MagicEntity entity : entities) {
        String newMappingKey = storage.buildKey(entity);
        if (pathCache.get(src.getType()).entrySet().stream()
            .anyMatch(
                entry ->
                    entry.getValue().equals(newMappingKey)
                        && !entry.getKey().equals(entity.getId()))) {
  ...
  public <T extends MagicEntity> boolean saveFile(T entity) {
    ...
    return writeLock(
        () -> {
          EventAction action =
              entity.getId() == null || !fileCache.containsKey(entity.getId())
                  ? EventAction.CREATE
                  : EventAction.SAVE;
          // 获取所在分组
          ...
          if (entity.getGroupId().contains(":")) {
            storage =
                (MagicResourceStorage<T>) this.storages.get(entity.getGroupId().split(":")[0]);
          } else {
            // 读取分组信息
            Group group = groupCache.get(entity.getGroupId());
          ...
          if (action == EventAction.CREATE) {
            ...
            entity.setUpdateBy(WebUtils.currentUserName());
            isTrue(
                !Constants.LOCK.equals(fileCache.get(entity.getId()).getLock()), RESOURCE_LOCKED);
    ...
    tree(group.getType())
        ...
        .forEach(
            g -> {
              ...
              fileCache.values().stream()
                  .filter(f -> f.getGroupId().equals(g.getId()))
                  .peek(entities::add)
                  .collect(Collectors.toList())
                  .forEach(
                      file -> {
        ...
        () -> {
          ...
          if (resource != null) {
            ...
            if (resource.exists() && resource.delete()) {
              ...
              GroupEvent event =
                  new GroupEvent(
                      groupCache.get(group.getId()).getType(), EventAction.DELETE, group);
              event.setEntities(deleteGroup(id));
              publisher.publishEvent(event);
              return true;
            }
  ...
  @Override
  public void export(String groupId, List<SelectedResource> resources, OutputStream os)
      throws IOException {
    if (StringUtils.isNotBlank(groupId)) {
      ...
      for (SelectedResource item : resources) {
        if ("root".equals(item.getType())) {
          ...
          MagicResourceStorage<? extends MagicEntity> storage = storages.get(group.getType());
          zos.putNextEntry(
              new ZipEntry(groupResource.getFilePath() + entity.getName() + storage.suffix()));
          zos.write(resource.read());
          zos.closeEntry();
        }
      }
  ...
  private void readAllResource(
      Resource root, Set<Group> groups, Set<MagicEntity> entities, boolean checked) {
    ...
    if (storage != null) {
      ...
      for (Resource file : root.files(storage.suffix())) {
        ...
        if (storage instanceof AbstractPathMagicResourceStorage) {
          mappingKey =
              ((AbstractPathMagicResourceStorage) storage)
                  .buildMappingKey((PathMagicEntity) entity, path);
        } else {
        ...
        if (checked) {
          ...
          fileCache.values().stream()
              ...
              .forEach(
                  it ->
                      isTrue(
                          !Objects.equals(it.getName(), entity.getName()),
                          RESOURCE_PATH_CONFLICT.format(entity.getName())));
        }
        // 自检
  ...
  public String getGroupName(String groupId) {
    return findGroups(groupId).stream().map(Group::getName).collect(Collectors.joining("/"));
  }
