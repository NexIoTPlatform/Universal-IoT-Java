 ...
 */
@Component
@ConditionalOnProperty(name = "nexiot.ide.debug.show-url", havingValue = "true", matchIfMissing = true)
@Order
@Slf4j
public class ApplicationUriPrinter implements CommandLineRunner {
  ...
  public void run(String... args) throws Exception {
    ...
    log.info(
        "服务启动成功，nexiot.ide.debug已内置启动! Access URLs:" + "\n\t接口本地地址: \t\t%s" + "\n\t接口外部地址: \t\t%s\n",
        localUrl, externUrl);
    ...
    log.info("\t可通过配置关闭输出: \tnexiot.ide.debug.show-url=false");
    log.info(
        "********************************************当前服务相关地址********************************************");
  }
}
