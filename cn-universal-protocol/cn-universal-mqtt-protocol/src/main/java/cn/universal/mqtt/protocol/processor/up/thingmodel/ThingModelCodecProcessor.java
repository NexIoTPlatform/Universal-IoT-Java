...
import cn.universal.persistence.base.BaseUPRequest.BaseUPRequestBuilder;
  ...
  private List<BaseUPRequest> tryProductCodec(MQTTUPRequest request) {
    try {
      ...
      if (CollUtil.isNotEmpty(decodedList)) {
        ...
        request.setStage(ProcessingStage.DECODED);
        BaseUPRequestBuilder newBuild = getBaseUPRequest(request.getIoTDeviceDTO());
        buildCodecNotNullBean(request, newBuild);
        return List.of(newBuild.build());
      }
