...
package cn.universal.http.protocol.converter;
 ...
 */
@Slf4j
@Component("httpConverter")
public class HttpDownRequestConverter extends AbstratIoTService
    implements DownRequestConverter<HttpDownRequest> {

  @Resource private HttpModuleInfo httpModuleInfo;

  @Override
  public HttpDownRequest convert(UnifiedDownlinkCommand command, DownlinkContext<?> context) {
    try {
      // 1. 加载产品信息
      ...
      log.error("[HTTP转换器] 参数验证失败: {}", e.getMessage());
      throw e;
    } catch (Exception e) {
      log.error("[HTTP转换器] 转换失败", e);
      throw new IllegalStateException("HTTP请求转换失败: " + e.getMessage(), e);
    }
  }
  ...
  private HttpDownRequest convertGatewaySubDevice(
    ...
    log.info(
        "[HTTP转换器] 网关子设备转换完成, 将使用网关设备通信: gatewayDeviceId={}, subDeviceId={}",
  ...
  private HttpDownRequest convertNormalDevice(
    ...
    log.debug(
        "[HTTP转换器] 转换完成: productKey={}, deviceId={}, cmd={}",
  ...
  private String encodeFunction(UnifiedDownlinkCommand command) {
    try {
      ...
      log.warn("[HTTP转换器] 编解码失败，返回原始数据: {}", e.getMessage());
  ...
  public String supportedProtocol() {
    return httpModuleInfo.getCode(); // "HTTP"
  }
  ...
  public void postConvert(
      UnifiedDownlinkCommand command, HttpDownRequest request, DownlinkContext<?> context) {
    // 后处理：记录转换信息到上下文
    context.setAttribute("convertTime", System.currentTimeMillis());
    context.setAttribute("protocolType", "HTTP");
