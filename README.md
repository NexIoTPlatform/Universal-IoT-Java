...
- 首先适合中大型企业，想做自己产品的，做B｜G项目
- 不再被某一个设备供应商、软件提供商卡脖子，漫天要价
- 大学生就能上手，无需养一大堆研发人员；研发、测试、维护都要时间
- 调试器给别人，让供应商来自主对接
- NexIoT用户间共享，产品、物模型、驱动内容，一键导出，即用即享
- 实时部署生效，0款到100款设备对接，几年都不用重启服务
...
| 多应用管理       | 应用管理模块提供完整的北向应用管理功能</br>为上层业务平台提供设备消息推送和接收业务平台设备控制操作的能力</br>支持账号创建多个应用</br>应用间数据订阅和南北向调用独立                                                                                | ✅ | ✅ |
| SDK支持(Java) | 设备管理: 支持设备的注册、查询、更新和删除</br>设备通信: 支持设备上线、下线状态管理</br>数据上报: 支持属性上报和事件上报</br>安全认证: 支持OAuth2客户端认证和密码认证</br>易于集成: 提供简洁的API接口，易于集成到现有系统                                          | ✅ | ✅ |
...
| TCP直连接入     | 支持自定义协议、Modbus等TCP通信方式</br>支持SNI TLS通配符域名接入</br>支持TCP集群，支持无限扩展节点</br>支持TCP粘包分包处理                                                                                          | ❌ | ✅ |
| Modbus协议    | 支持Modbus RTU/TCP协议</br>支持Modbus设备接入和数据处理</br> **注意：开源版开源通过MQTT方式**                                                                                                        | ✅ | ✅ |
| 天翼物联        | 支持与天翼物联AIOT无缝接入</br>产品管理、设备管理全部在nexiot统一管理                                                                                                                                | ❌ | ✅ |
| 视频接入        | 乐橙云视频接入（企业赠送）</br>GB28181视频协议支持（roadmap内）                                                                                                                                 | ❌ | ✅ |
| 集群部署        | 支持多节点集群部署</br>支持分布式锁、负载均衡</br>支持高可用、水平扩展                                                                                                                                  | ✅ | ✅ |
| 日志存储        | 支持动态切换日志存储方式</br>支持None(不存) / Mysql / ClickHouse / IoTDB                                                                                                                  | ✅ | ✅ |
...
| **🔧 专属技术支持** | • 各类对接支持<br>• 使用指导<br>• 部署方案<br>• 解决方案 | **¥300/4小时** | 远程支持            | 联系技术                                                         |
...
请遵循AGPL3.0开源协议，商业使用需获得授权，保留企业授权信息
