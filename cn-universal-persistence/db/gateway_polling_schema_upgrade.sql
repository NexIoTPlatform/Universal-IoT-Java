...
INNER JOIN `iot_device` d ON c.`gateway_device_id` = d.`device_id`
...
SHOW INDEX FROM `iot_gateway_polling_config` WHERE `Key_name` = 'uk_device';

-- 验证 iot_gateway_polling_command 表的字段
DESC `iot_gateway_polling_command`;

-- 验证 iot_gateway_polling_command 表的索引
SHOW INDEX FROM `iot_gateway_polling_command` WHERE `Key_name` = 'idx_gateway';

-- 验证数据完整性 (应该返回 0)
SELECT COUNT(*) AS missing_product_key_count
FROM `iot_gateway_polling_command`
WHERE `gateway_product_key` IS NULL OR `gateway_product_key` = '';
