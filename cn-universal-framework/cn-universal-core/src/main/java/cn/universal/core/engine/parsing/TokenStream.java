...
public class TokenStream {
    ...
    if (!match(true, types)) {
      ...
      if (span == null) {
        MagicScriptError.error(
            "期待 '"
                + Stream.of(types).map(TokenType::getError).collect(Collectors.joining("','"))
                + "', 但是流已经遍历完毕",
            this);
      } else {
        MagicScriptError.error(
  ...
  public List<Span> comments() {
    return tokens.stream()
        .filter(it -> it.getType() == TokenType.Comment)
        .map(Token::getSpan)
  ...
  public Token expect(TokenType type) {
    if (!match(type, true)) {
      ...
      if (span == null) {
        ...
        MagicScriptError.error("期待 '" + type.getError() + "', 获得 '" + token.getText() + "'",
            span);
      }
      return null; // 执行不到这里
    } else {
