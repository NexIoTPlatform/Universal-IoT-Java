 ...
 * {@code visitTypeAnnotation} | {@code visitAttribute} )* [ {@code visitCode} ( {@code visitFrame}
 * | {@code visit<i>X</i>Insn} | {@code visitLabel} | {@code visitInsnAnnotation} |
 * {@code visitTryCatchBlock} | {@code visitTryCatchAnnotation} | {@code visitLocalVariable} |
 * {@code visitLocalVariableAnnotation} | {@code visitLineNumber} )* {@code visitMaxs} ]
 * {@code visitEnd}. In addition, the {@code visit<i>X</i>Insn} and {@code visitLabel} methods must
 * be called in the sequential order of the bytecode instructions of the visited code,
 * {@code visitInsnAnnotation} must be called <i>after</i> the annotated instruction,
 * {@code visitTryCatchBlock} must be called
 ...
 * been visited, and the {@code visitLocalVariable}, {@code visitLocalVariableAnnotation} and
 * {@code visitLineNumber} methods must be called <i>after</i> the labels passed as arguments have
 * been visited. @Author Eric Bruneton
 */
public abstract class MethodVisitor {
  ...
  /**
   * Visits the default value of this annotation processer method.
   *
   * @return a visitor to the visit the actual default value of this annotation processer method, or
   * {@literal null} if this visitor is not interested in visiting this default value. The 'name'
   ...
   *                   {@link TypeReference#METHOD_TYPE_PARAMETER},
   ...
   *                       must be less or equal than the number of parameter types in the method
   *                       descriptor. It can be strictly less when a method has synthetic
   *                       parameters and when these parameters are ignored when computing parameter
   *                       indices for the purpose of parameter annotations (see
   *                       https://docs.oracle.com/javase/specs/jvms/se9/html/jvms-4.html#jvms-4.7.18).
   * @param visible        {@literal true} to define the number of method parameters that can have
   *                       annotations visible at runtime, {@literal false} to define the number of
   ...
   *                   parameters in the method descriptor, and strictly smaller than the parameter
   *                   count specified in {@link #visitAnnotableParameterCount}. Important note:
   ...
   *                 {@link Opcodes#UNINITIALIZED_THIS} (long and double are represented by a single
   *                 element). Reference types are represented by String objects (representing
   ...
   *                SIPUSH or NEWARRAY.
   * @param operand the operand of the instruction to be visited.<br> When opcode is BIPUSH, operand
   *                value should be between Byte.MIN_VALUE and Byte.MAX_VALUE.
   *                <br>
   ...
   * @param name        the method's name.
   * @param descriptor  the method's descriptor (see {@link Type}).
   * @param isInterface if the method's owner class is an processer.
   */
  public void visitMethodInsn(
      final int opcode,
      final String owner,
   ...
   *                                 {@link Double}, {@link String}, {@link Type}, {@link Handle} or
   *                                 {@link ConstantDynamic} value. This method is allowed to modify
   *                                 the content of the array so a caller should expect that this
   *                                 array may change.
   */
   ...
   *              is 51 or a {@link ConstantDynamic} for a constant dynamic for classes whose
   *              version is 55.
   */
   ...
   *                   or static inner type within 'typeRef'. May be {@literal null} if the
   *                   annotation targets 'typeRef' as a whole.
   * @param start      the fist instructions corresponding to the continuous ranges that make the
   *                   scope of this local variable (inclusive).
   * @param end        the last instructions corresponding to the continuous ranges that make the
   *                   scope of this local variable (exclusive). This array must have the same size
   *                   as the 'start' array.
   * @param index      the local variable's index in each range. This array must have the same size
   *                   as the 'start' array.
   * @param descriptor the class descriptor of the annotation class.
   * @param visible    {@literal true} if the annotation is visible at runtime.
   * @return a visitor to visit the annotation values, or {@literal null} if this visitor is not
